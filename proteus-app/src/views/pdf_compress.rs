//! PDF Compress (PRD §9) — single-file PDF recompression.
...
use gpui_component::*;
use proteus_core::pdf::compress_pdf;

use super::common::{self, Filters, ToolState};

pub struct CompressPdfView {
    state: ToolState,
    this: Entity<Self>,
}

impl CompressPdfView {
    ...
    fn process_action(this: &Entity<Self>, cx: &mut App) {
        ...
        cx.spawn(async move |cx| {
            let result = compress_pdf(&input);
            this.update(cx, |s, cx| match result {
                Ok(out) => {
                    s.state.output_bytes = Some(Arc::from(out));
                    s.state.output_name = Some("compressed.pdf".into());
    ...
    fn save_action(this: &Entity<Self>, window: &mut Window, cx: &mut App) {
        ...
        let Some(path) = common::pick_output_file(window, "compressed.pdf", Filters::Pdf) else {
            return;
        };
...
impl Render for CompressPdfView {
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .p_6()
            .gap_4()
            .child(div().text_xl().child("Compress PDF"))
            .child(
                div()
                    .text_sm()
                    .text_color(gpui_component::gray(400))
                    .child("Recompress the PDF to reduce its size (lossless where possible)."),
            )
            .child(
                Button::new("pick")
                    .label("Open PDF…")
